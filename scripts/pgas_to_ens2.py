...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate synthetic datasets from PGAS parameter samples and train ENS2 models."
    )
    p.add_argument(
        "--param-samples",
        type=Path,
    ...
    p.add_argument(
        ...
        help="Output directory for trained ENS2 models.",
    )
    ...
    p.add_argument("--neuron-type", type=str, default="Exc", choices=["Exc", "exc", "Inh", "inh"])
    p.add_argument("--sampling-rate", type=float, default=60.0)
    p.add_argument("--smoothing-std", type=float, default=0.025)
    p.add_argument("--model-prefix", type=str, default="bio_ml_ens2")
    return p.parse_args()
...
def main() -> None:
    ...
    train_cfg = default_ens2_train_config()
    train_cfg.update(
        {
    ...
    records = train_models_for_bundles(
        ...
        ens2_train_config=train_cfg,
    )
    print(f"Generated {len(bundles)} synthetic bundle(s).")
    print(f"Trained {len(records)} ENS2 model(s).")
